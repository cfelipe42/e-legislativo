...
const VotingSession: React.FC<VotingSessionProps> = ({ 
  ...
  const myId = '1'; 
  const myData = councilmen.find(c => c.id === myId);
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
               ...
               {hasCouncilPowers && (
                 <button 
                   ...
                   className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2 ${myData?.isRequestingFloor ? 'bg-amber-500 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                 >
                   <i className={`fa-solid ${myData?.isRequestingFloor ? 'fa-microphone-lines animate-pulse' : 'fa-microphone'}`}></i>
                   {myData?.isRequestingFloor ? 'Solicitação Ativa' : 'Pedir a Palavra'}
                 </button>
               )}
            </div>
                    ...
                    <div className="space-y-3">
                       {speakersPending.length > 0 ? (
                         speakersPending.map(s => (
                           <div key={s.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100 group animate-fadeIn">
                              ...
                              <button 
                                ...
                                className="w-8 h-8 flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl transition-all shadow-md active:scale-90"
